...
// 몬스터가 받은 피해를 데미지 텍스트로 화면에 띄워줍니다.
...
// 데미지를 입을 때 화면에 재생할 효과를 지정합니다.
	void DamageEffect(string effectPrefab)
	{
		if(effectPrefab == "")
		{
			return;
		}else if(effectPrefab == "SkillAttack1"){
		...
		}else if(effectPrefab == "SkillAttack2"){
			GameObject effect = SkillAttack2Pool.Instance.GetObject();
			effect.transform.position = transform.position+ new Vector3(0f,0.5f,-0.5f);
			effect.GetComponent<SkillAttack2>().Play();
		}

			//GameObject damageObj = DamageTextPool.Instance.GetObject();
// 데미지 효과를 화면에 재생합니다.	
			//Instantiate(Resources.Load(effectPrefab), transform.position+ new Vector3(0f,0.5f,-0.5f), transform.rotation);
...
// 타격시 사운드를 재생합니다.
	void PlaySound(string audio)
	{
		if(audio != "")
		{
			...
			AudioSource.PlayClipAtPoint(Resources.Load(audio) as AudioClip, transform.position, 1f);
		}
	}
...
// 오브젝트 풀에 몬스터를 되돌려놓도록 하겠습니다.
// Destroy (gameObject, 2f);

		StartCoroutine(StartRelease(2f));
	}

	IEnumerator StartRelease(float time)
	{
		yield return new WaitForSeconds(time);
		SlimePool.Instance.ReleaseObject(gameObject);
	}
