...
namespace Boomlagoon.JSON {
	...
	public class JSONObject : IEnumerable<KeyValuePair<string, JSONValue>> {
		...
		public static bool isLongNumber = false;
		...
		public System.Collections.Generic.ICollection<string> KeyList() {
			return values.Keys;
		}
			...
			for (var startPosition = 0; startPosition < jsonString.Length; ++startPosition) {
				...
				switch (state) {
					...
					case JSONParsingState.Number:
						// Chris Fix Start 2013.7.25 
						
						var endPosition = startPosition + 1;
			
						for (;
							endPosition < jsonString.Length && jsonString[endPosition] != ',' && jsonString[endPosition] != ']' && jsonString[endPosition] != '}';
							++endPosition) ;
						if(endPosition - startPosition >15){
							isLongNumber = true;
							state = JSONParsingState.String;
							break;
						}
						// Chris Fix End 2013.7.25 
					...
					case JSONParsingState.String:
						string str = null;
						//Debug.Log ("isLongNumber: "+isLongNumber);
						//Debug.Log (jsonString.Substring(startPosition));
						if(isLongNumber){
							str = ParseLongToString(jsonString, ref startPosition);
						//Debug.Log ("str: "+str);
						}else{
							str = ParseString(jsonString, ref startPosition);
							if (str == null) {
		...
		private static string ParseLongToString(string str, ref int startPosition) {
